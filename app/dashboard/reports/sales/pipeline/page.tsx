...
export default function PipelinePage() {
    ...
    const loadData = async () => {
        ...
        setLoading(true);
        try {
            const result = await reportsService.getSalesPipeline({
                orgId: profile.orgId,
    ...
    return (
        <ReportPageShell
            ...
            description="Track deals by stage, value, and conversion rates."
            dateRange={dateRange}
            ...
            {data && (
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <ChartCard
                            title="Deals by Stage (Funnel)"
                            type="bar"
                            data={data.series}
                            series={[{ key: "value", color: "#8b5cf6", name: "Deals Count" }]}
                        />
                        <ChartCard
                            title="Pipeline Value by Stage"
                            type="bar"
                            data={data.series}
                            series={[{ key: "amount", color: "#3b82f6", name: "Value ($)" }]}
                        />
                    </div>
