...
export default function TasksPage() {
    ...
    const [columnVisibility, setColumnVisibility] = useState<Record<ColumnKey, boolean>>({
        name: true, status: true, startDate: true, dueDate: true, priority: true
    });
    ...
    const handleExport = () => {
        ...
        const csv = ["Name,Status,Priority,Start Date,Due Date", ...dataToExport.map(t => `"${t.name}","${t.status}","${t.priority}","${formatDate(t.startDate)}","${formatDate(t.dueDate)}"`)].join("\n");
    ...
    const handleKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
        ...
        switch (e.key) {
            case "ArrowDown": e.preventDefault(); setFocusedRowIndex(Math.min(focusedRowIndex + 1, paginatedTasks.length - 1)); break;
            case "ArrowUp": e.preventDefault(); setFocusedRowIndex(Math.max(focusedRowIndex - 1, 0)); break;
            case " ": e.preventDefault(); toggleSelect(paginatedTasks[focusedRowIndex].id); break;
        }
    ...
    if (customerLoading || tasksLoading) {
        return <div className="p-8 flex items-center gap-2"><Loader2 className="h-5 w-5 animate-spin" />Loading tasks...</div>;
    }
            ...
            <div className="space-y-4" onKeyDown={handleKeyDown} tabIndex={0} ref={tableRef}>
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="bg-white rounded-lg border p-4">
                        ...
                        <div className="text-2xl font-bold text-green-600">{taskStats['complete'] || 0}</div>
                    </div>
                    <div className="bg-white rounded-lg border p-4">
                        <div className="text-sm text-gray-500">Open</div>
                        <div className="text-2xl font-bold text-blue-600">{taskStats.total - (taskStats['complete'] || 0)}</div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="relative w-64">
                        ...
                        <Input placeholder="Search..." className="pl-9" autoComplete="new-password" name="tasks-search-nofill" data-lpignore="true" data-1p-ignore="true" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                </div>

                {/* Top Pagination */}
                {processedTasks.length > 0 && (
                    <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalRecords={processedTasks.length} startRecord={startRecord} endRecord={endRecord} />
                )}

                {/* Selection Banner */}
                {selectedIds.length > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-md p-3 flex items-center justify-between">
                        <span className="text-blue-800 text-sm font-medium">{selectedIds.length} task{selectedIds.length > 1 ? "s" : ""} selected</span>
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={handleSelectAll}>Select All ({processedTasks.length})</Button>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {paginatedTasks.length === 0 ? (
                                <TableRow><TableCell colSpan={visibleColumns.length + 2} className="text-center py-8 text-gray-500">{searchQuery ? "No tasks match your search." : `No tasks found for ${customer?.company || "this customer"}.`}</TableCell></TableRow>
                            ) : (
                                paginatedTasks.map((task, index) => (
                                    <TableRow key={task.id} className={`group hover:bg-gray-50 ${focusedRowIndex === index ? "bg-blue-50" : ""} ${selectedIds.includes(task.id) ? "bg-blue-50/50" : ""}`} onClick={() => setFocusedRowIndex(index)}>
                                        <TableCell className={ROW_DENSITY_STYLES[rowDensity]}>
                                            <Checkbox checked={selectedIds.includes(task.id)} onCheckedChange={() => toggleSelect(task.id)} onClick={(e) => e.stopPropagation()} />
                                        </TableCell>
                                        {visibleColumns.map(col => (
                                            <TableCell key={col.key} className={ROW_DENSITY_STYLES[rowDensity]}>
                                                ...
                                                {col.key === "status" && (
                                                    <Badge className={`${getStatusBadge(task.status)} font-normal`}>
                                                        <HighlightText text={formatStatus(task.status)} search={searchQuery} />
                                                    </Badge>
                                                )}
                                                {col.key === "priority" && (
                                                    <Badge className={`${getPriorityBadge(task.priority)} font-normal`}>
                                                        <HighlightText text={formatStatus(task.priority)} search={searchQuery} />
                                                    </Badge>
                                                )}
                                        ...
                                        <TableCell className={`${ROW_DENSITY_STYLES[rowDensity]} overflow-visible`}>
                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Tooltip><TooltipTrigger asChild><Link href={`/dashboard/tasks/${task.id}`}><Button variant="ghost" size="icon" className="h-7 w-7"><Eye className="h-4 w-4 text-gray-500" /></Button></Link></TooltipTrigger><TooltipContent>View</TooltipContent></Tooltip>
                                                <Tooltip><TooltipTrigger asChild><Link href={`/dashboard/tasks/${task.id}/edit`}><Button variant="ghost" size="icon" className="h-7 w-7"><Pencil className="h-4 w-4 text-gray-500" /></Button></Link></TooltipTrigger><TooltipContent>Edit</TooltipContent></Tooltip>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ))
