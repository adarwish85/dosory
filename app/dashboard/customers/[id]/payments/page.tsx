...
export default function PaymentsPage() {
    ...
    const handleExport = () => {
        const dataToExport =
            selectedIds.length > 0 ? payments.filter((p) => selectedIds.includes(p.id)) : processedPayments;
        const csv = [
            "Date,Invoice,Amount,Payment Mode,Transaction ID",
            ...dataToExport.map(
                (p) =>
                    `"${formatDate(p.date)}","${p.invoiceNumber || "-"}","${p.amount}","${p.paymentMode || "-"}","${p.transactionId || "-"}"`
            ),
        ].join("\n");
    ...
    const handleKeyDown = useCallback(
        (e: KeyboardEvent<HTMLDivElement>) => {
            ...
            switch (e.key) {
                case "ArrowDown":
                    e.preventDefault();
                    setFocusedRowIndex(Math.min(focusedRowIndex + 1, paginatedPayments.length - 1));
                    break;
                case "ArrowUp":
                    e.preventDefault();
                ...
                case " ":
                    e.preventDefault();
                    toggleSelect(paginatedPayments[focusedRowIndex].id);
                    break;
            }
        },
                        ...
                        <TableBody>
                            {paginatedPayments.length === 0 ? (
                                ...
                                paginatedPayments.map((payment, index) => (
                                    <TableRow
                                        key={payment.id}
                                        className={`group hover:bg-gray-50 ${focusedRowIndex === index ? "bg-blue-50" : ""} ${selectedIds.includes(payment.id) ? "bg-blue-50/50" : ""}`}
