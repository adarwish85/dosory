...
export default function ProjectsPage() {
    ...
    const [columnVisibility, setColumnVisibility] = useState<Record<ColumnKey, boolean>>({
        name: true, progress: true, startDate: true, deadline: true, status: true
    });
        ...
        if (sortKey && sortDirection) {
            result.sort((a, b) => {
                ...
                switch (sortKey) {
                    ...
                    case "deadline": aVal = a.deadline?.toMillis?.() || 0; bVal = b.deadline?.toMillis?.() || 0; break;
    ...
    // Export
    const handleExport = () => {
        const dataToExport = selectedIds.length > 0 ? projects.filter(p => selectedIds.includes(p.id)) : processedProjects;
        const csv = ["Name,Progress,Start Date,Deadline,Status", ...dataToExport.map(p => `"${p.name}","${p.progress || 0}%","${formatDate(p.startDate)}","${formatDate(p.deadline)}","${p.status}"`)].join("\n");
    ...
    const handleKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
        ...
        switch (e.key) {
            case "ArrowDown": e.preventDefault(); setFocusedRowIndex(Math.min(focusedRowIndex + 1, paginatedProjects.length - 1)); break;
            case "ArrowUp": e.preventDefault(); setFocusedRowIndex(Math.max(focusedRowIndex - 1, 0)); break;
            case " ": e.preventDefault(); toggleSelect(paginatedProjects[focusedRowIndex].id); break;
        }
    ...
    if (customerLoading || projectsLoading) {
        return <div className="p-8 flex items-center gap-2"><Loader2 className="h-5 w-5 animate-spin" />Loading projects...</div>;
    }
            ...
            <div className="space-y-4" onKeyDown={handleKeyDown} tabIndex={0} ref={tableRef}>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="relative w-64">
                        ...
                        <Input placeholder="Search..." className="pl-9" autoComplete="new-password" name="projects-search-nofill" data-lpignore="true" data-1p-ignore="true" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                </div>

                {/* Top Pagination */}
                {processedProjects.length > 0 && (
                    <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalRecords={processedProjects.length} startRecord={startRecord} endRecord={endRecord} />
                )}

                {/* Selection Banner */}
                {selectedIds.length > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-md p-3 flex items-center justify-between">
                        <span className="text-blue-800 text-sm font-medium">{selectedIds.length} project{selectedIds.length > 1 ? "s" : ""} selected</span>
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={handleSelectAll}>Select All ({processedProjects.length})</Button>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {paginatedProjects.length === 0 ? (
                                <TableRow><TableCell colSpan={visibleColumns.length + 2} className="text-center py-8 text-gray-500">{searchQuery ? "No projects match your search." : `No projects found for ${customer?.company || "this customer"}.`}</TableCell></TableRow>
                            ) : (
                                paginatedProjects.map((project, index) => (
                                    <TableRow key={project.id} className={`group hover:bg-gray-50 ${focusedRowIndex === index ? "bg-blue-50" : ""} ${selectedIds.includes(project.id) ? "bg-blue-50/50" : ""}`} onClick={() => setFocusedRowIndex(index)}>
                                        <TableCell className={ROW_DENSITY_STYLES[rowDensity]}>
                                            <Checkbox checked={selectedIds.includes(project.id)} onCheckedChange={() => toggleSelect(project.id)} onClick={(e) => e.stopPropagation()} />
                                        </TableCell>
                                        {visibleColumns.map(col => (
                                            <TableCell key={col.key} className={ROW_DENSITY_STYLES[rowDensity]}>
                                                ...
                                                {col.key === "startDate" && <span>{formatDate(project.startDate)}</span>}
                                                {col.key === "deadline" && <span>{formatDate(project.deadline)}</span>}
                                                {col.key === "status" && (
                                                    <Badge className={`${getStatusBadge(project.status)} font-normal`}>
                                                        <HighlightText text={formatStatus(project.status)} search={searchQuery} />
                                                    </Badge>
                                                )}
                                            </TableCell>
                                        ))}
                                        <TableCell className={`${ROW_DENSITY_STYLES[rowDensity]} overflow-visible`}>
                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Tooltip><TooltipTrigger asChild><Link href={`/dashboard/projects/${project.id}`}><Button variant="ghost" size="icon" className="h-7 w-7"><Eye className="h-4 w-4 text-gray-500" /></Button></Link></TooltipTrigger><TooltipContent>View</TooltipContent></Tooltip>
                                                <Tooltip><TooltipTrigger asChild><Link href={`/dashboard/projects/${project.id}/edit`}><Button variant="ghost" size="icon" className="h-7 w-7"><Pencil className="h-4 w-4 text-gray-500" /></Button></Link></TooltipTrigger><TooltipContent>Edit</TooltipContent></Tooltip>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ))
