...
export default function InvoicesPage() {
    ...
    const openCreateWizard = () => {
        ...
        setWizardInvoiceId(undefined);
        setWizardOpen(true);
    };
    const openViewWizard = (id: string) => {
        setWizardMode("view");
        setWizardInvoiceId(id);
        setWizardOpen(true);
    };
    ...
    const processedInvoices = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter(
                (inv) => String(inv.number).includes(searchQuery) || inv.status?.toLowerCase().includes(lowerQuery)
            );
        }
    ...
    const handleExport = () => {
        const dataToExport =
            selectedIds.length > 0 ? invoices.filter((i) => selectedIds.includes(i.id)) : processedInvoices;
        const csv = [
            "Invoice #,Amount,Tax,Date,Due Date,Status",
            ...dataToExport.map(
                (i) =>
                    `"INV-${String(i.number).padStart(6, "0")}","${i.total}","${i.taxTotal || 0}","${formatDate(i.date)}","${formatDate(i.dueDate)}","${i.status}"`
            ),
        ].join("\n");
    ...
    const handleKeyDown = useCallback(
        (e: KeyboardEvent<HTMLDivElement>) => {
            ...
            switch (e.key) {
                case "ArrowDown":
                    e.preventDefault();
                    setFocusedRowIndex(Math.min(focusedRowIndex + 1, paginatedInvoices.length - 1));
                    break;
                case "ArrowUp":
                    e.preventDefault();
                ...
                case " ":
                    e.preventDefault();
                    toggleSelect(paginatedInvoices[focusedRowIndex].id);
                    break;
                case "Enter":
                    e.preventDefault();
                    window.location.href = `/dashboard/invoices/${paginatedInvoices[focusedRowIndex].id}`;
                    break;
            }
        },
                    ...
                    items={[
                        {
                            ...
                            amount: formatInvoiceCurrency(invoiceStats?.totalDue || 0),
                            color: "orange",
                        },
                        {
                            label: t("invoices.stats.pastDue"),
                            amount: formatInvoiceCurrency(invoiceStats?.amountsByStatus?.overdue || 0),
                            color: "default",
                        },
                        {
                            label: t("invoices.stats.totalPaid"),
                            amount: formatInvoiceCurrency(invoiceStats?.totalPaid || 0),
                            color: "green",
                        },
                    ]}
                />
                        ...
                        <TableBody>
                            {paginatedInvoices.length === 0 ? (
                                ...
                                paginatedInvoices.map((invoice, index) => (
                                    <TableRow
                                        key={invoice.id}
                                        className={`group hover:bg-gray-50 ${focusedRowIndex === index ? "bg-blue-50" : ""} ${selectedIds.includes(invoice.id) ? "bg-blue-50/50" : ""}`}
                                    ...
                                    >
                                        ...
                                        {visibleColumns.map((col) => (
                                            <TableCell key={col.key} className={ROW_DENSITY_STYLES[rowDensity]}>
                                                ...
                                                {col.key === "tax" && (
                                                    <span>{formatInvoiceCurrency(invoice.taxTotal || 0)}</span>
                                                )}
                                                ...
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Button
                                                            ...
                                                            size="icon"
                                                            className="h-7 w-7"
                                                            onClick={() => openViewWizard(invoice.id)}
                                                        >
                                                ...
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Button
                                                            ...
                                                            size="icon"
                                                            className="h-7 w-7"
                                                            onClick={() => openEditWizard(invoice.id)}
                                                        >
