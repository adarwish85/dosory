    ...
    const processedCreditNotes = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter(
                (cn) => String(cn.number).includes(searchQuery) || cn.status?.toLowerCase().includes(lowerQuery)
            );
        }
    ...
    const handleKeyDown = useCallback(
        (e: KeyboardEvent<HTMLDivElement>) => {
            ...
            switch (e.key) {
                case "ArrowDown":
                    e.preventDefault();
                    setFocusedRowIndex(Math.min(focusedRowIndex + 1, paginatedCreditNotes.length - 1));
                    break;
                case "ArrowUp":
                    e.preventDefault();
                ...
                case " ":
                    e.preventDefault();
                    toggleSelect(paginatedCreditNotes[focusedRowIndex].id);
                    break;
            }
        },
            ...
            <div className="space-y-4" onKeyDown={handleKeyDown} tabIndex={0} ref={tableRef}>
                ...
                {processedCreditNotes.length > 0 && (
                    <Pagination
                        ...
                        totalRecords={processedCreditNotes.length}
                        ...
                        <TableBody>
                            {paginatedCreditNotes.length === 0 ? (
                                ...
                                paginatedCreditNotes.map((creditNote, index) => (
                                    <TableRow
                                        key={creditNote.id}
                                        className={`group hover:bg-gray-50 ${focusedRowIndex === index ? "bg-blue-50" : ""} ${selectedIds.includes(creditNote.id) ? "bg-blue-50/50" : ""}`}
                                        onClick={() => setFocusedRowIndex(index)}
                                    >
                                        <TableCell className={ROW_DENSITY_STYLES[rowDensity]}>
                                            <Checkbox
                                                checked={selectedIds.includes(creditNote.id)}
                                                onCheckedChange={() => toggleSelect(creditNote.id)}
                                        ...
                                        {visibleColumns.map((col) => (
                                            <TableCell key={col.key} className={ROW_DENSITY_STYLES[rowDensity]}>
                                                ...
                                                {col.key === "status" && (
                                                    <Badge
                                                        className={`${getStatusBadge(creditNote.status)} font-normal`}
                                                    >
                                                        <HighlightText
                                                            text={formatStatus(creditNote.status)}
                                                            search={searchQuery}
                                                        />
                                                    </Badge>
                                                )}
