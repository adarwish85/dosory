...
type ColumnKey = "number" | "amount" | "date" | "status";
...
export default function CreditNotesPage() {
    ...
    const [columnVisibility, setColumnVisibility] = useState<Record<ColumnKey, boolean>>({ number: true, amount: true, date: true, status: true });
    ...
    const processedCreditNotes = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter(cn =>
                String(cn.number).includes(searchQuery) ||
                cn.status?.toLowerCase().includes(lowerQuery)
            );
        }
    ...
    const closedTotal = creditNotes.filter(cn => cn.status === "closed").reduce((sum, cn) => sum + (cn.amount || 0), 0);

    // Pagination
    const totalPages = Math.max(1, Math.ceil(processedCreditNotes.length / recordsPerPage));
    const startIndex = (currentPage - 1) * recordsPerPage;
    const paginatedCreditNotes = processedCreditNotes.slice(startIndex, startIndex + recordsPerPage);
    const startRecord = processedCreditNotes.length === 0 ? 0 : startIndex + 1;
    const endRecord = Math.min(startIndex + recordsPerPage, processedCreditNotes.length);
    ...
    const isAllSelected = paginatedCreditNotes.length > 0 && paginatedCreditNotes.every(cn => selectedIds.includes(cn.id));
    const isSomeSelected = paginatedCreditNotes.some(cn => selectedIds.includes(cn.id)) && !isAllSelected;

    // Export
    const handleExport = () => {
        const dataToExport = selectedIds.length > 0 ? creditNotes.filter(cn => selectedIds.includes(cn.id)) : processedCreditNotes;
        const csv = ["Credit Note #,Amount,Date,Status", ...dataToExport.map(cn => `"CN-${cn.number}","${cn.amount}","${formatDate(cn.date)}","${cn.status}"`)].join("\n");
    ...
    const handleKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
        if (focusedRowIndex === null || paginatedCreditNotes.length === 0) return;
        switch (e.key) {
            case "ArrowDown": e.preventDefault(); setFocusedRowIndex(Math.min(focusedRowIndex + 1, paginatedCreditNotes.length - 1)); break;
            case "ArrowUp": e.preventDefault(); setFocusedRowIndex(Math.max(focusedRowIndex - 1, 0)); break;
            case " ": e.preventDefault(); toggleSelect(paginatedCreditNotes[focusedRowIndex].id); break;
        }
    ...
    if (customerLoading || creditNotesLoading) {
        return <div className="p-8 flex items-center gap-2"><Loader2 className="h-5 w-5 animate-spin" />Loading credit notes...</div>;
    }
            ...
            <div className="space-y-4" onKeyDown={handleKeyDown} tabIndex={0} ref={tableRef}>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="relative w-64">
                        ...
                        <Input placeholder="Search..." className="pl-9" autoComplete="new-password" name="creditnotes-search-nofill" data-lpignore="true" data-1p-ignore="true" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                </div>

                {/* Top Pagination */}
                {processedCreditNotes.length > 0 && (
                    <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalRecords={processedCreditNotes.length} startRecord={startRecord} endRecord={endRecord} />
                )}

                {/* Selection Banner */}
                {selectedIds.length > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-md p-3 flex items-center justify-between">
                        <span className="text-blue-800 text-sm font-medium">{selectedIds.length} credit note{selectedIds.length > 1 ? "s" : ""} selected</span>
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={handleSelectAll}>Select All ({processedCreditNotes.length})</Button>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {paginatedCreditNotes.length === 0 ? (
                                <TableRow><TableCell colSpan={visibleColumns.length + 2} className="text-center py-8 text-gray-500">{searchQuery ? "No credit notes match your search." : `No credit notes found for ${customer?.company || "this customer"}.`}</TableCell></TableRow>
                            ) : (
                                paginatedCreditNotes.map((creditNote, index) => (
                                    <TableRow key={creditNote.id} className={`group hover:bg-gray-50 ${focusedRowIndex === index ? "bg-blue-50" : ""} ${selectedIds.includes(creditNote.id) ? "bg-blue-50/50" : ""}`} onClick={() => setFocusedRowIndex(index)}>
                                        <TableCell className={ROW_DENSITY_STYLES[rowDensity]}>
                                            <Checkbox checked={selectedIds.includes(creditNote.id)} onCheckedChange={() => toggleSelect(creditNote.id)} onClick={(e) => e.stopPropagation()} />
                                        </TableCell>
                                        {visibleColumns.map(col => (
                                            <TableCell key={col.key} className={ROW_DENSITY_STYLES[rowDensity]}>
                                                {col.key === "number" && (
                                                    <span className="text-blue-600 font-medium">
                                                        <HighlightText text={`CN-${creditNote.number}`} search={searchQuery} />
                                                    </span>
                                                )}
                                                {col.key === "amount" && <span className="font-medium">{formatCurrency(creditNote.amount)}</span>}

                                                {col.key === "date" && <span>{formatDate(creditNote.date)}</span>}
                                                {col.key === "status" && (
                                                    <Badge className={`${getStatusBadge(creditNote.status)} font-normal`}>
                                                        <HighlightText text={formatStatus(creditNote.status)} search={searchQuery} />
                                                    </Badge>
                                                )}
                                            </TableCell>
                                        ))}
