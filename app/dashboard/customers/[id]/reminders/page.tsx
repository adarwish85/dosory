...
export default function RemindersPage() {
    ...
    const [columnVisibility, setColumnVisibility] = useState<Record<ColumnKey, boolean>>({
        status: true, title: true, description: true, date: true
    });
        ...
        if (sortKey && sortDirection) {
            result.sort((a, b) => {
                ...
                switch (sortKey) {
                    ...
                    case "status": aVal = getDateStatus(a.date); bVal = getDateStatus(b.date); break;
                    default: return 0;
                }
    ...
    // Export
    const handleExport = () => {
        const dataToExport = selectedIds.length > 0 ? reminders.filter(r => selectedIds.includes(r.id)) : processedReminders;
        const csv = ["Title,Description,Date,Status,Is Notified", ...dataToExport.map(r => `"${r.title}","${r.description || ''}","${formatDate(r.date)}","${formatStatus(getDateStatus(r.date))}","${r.isNotified}"`)].join("\n");
        const blob = new Blob([csv], { type: "text/csv" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a"); a.href = url; a.download = "reminders-export.csv"; a.click();
    ...
    const handleKeyDown = useCallback((e: KeyboardEvent<HTMLDivElement>) => {
        ...
        switch (e.key) {
            case "ArrowDown": e.preventDefault(); setFocusedRowIndex(Math.min(focusedRowIndex + 1, paginatedReminders.length - 1)); break;
            case "ArrowUp": e.preventDefault(); setFocusedRowIndex(Math.max(focusedRowIndex - 1, 0)); break;
            case " ": e.preventDefault(); toggleSelect(paginatedReminders[focusedRowIndex].id); break;
        }
    ...
    if (customerLoading || remindersLoading) {
        return <div className="p-8 flex items-center gap-2"><Loader2 className="h-5 w-5 animate-spin" />Loading reminders...</div>;
    }
            ...
            <div className="space-y-4" onKeyDown={handleKeyDown} tabIndex={0} ref={tableRef}>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="relative w-64">
                        ...
                        <Input placeholder="Search reminders..." className="pl-9" autoComplete="new-password" name="reminders-search-nofill" data-lpignore="true" data-1p-ignore="true" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
                </div>

                {/* Top Pagination */}
                {processedReminders.length > 0 && (
                    <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalRecords={processedReminders.length} startRecord={startRecord} endRecord={endRecord} />
                )}

                {/* Selection Banner */}
                {selectedIds.length > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-md p-3 flex items-center justify-between">
                        <span className="text-blue-800 text-sm font-medium">{selectedIds.length} reminder{selectedIds.length > 1 ? "s" : ""} selected</span>
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={handleSelectAll}>Select All ({processedReminders.length})</Button>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {paginatedReminders.length === 0 ? (
                                <TableRow><TableCell colSpan={visibleColumns.length + 2} className="text-center py-8 text-gray-500">{searchQuery ? "No reminders match your search." : `No reminders found for ${customer?.company || "this customer"}.`}</TableCell></TableRow>
                            ) : (
                                paginatedReminders.map((reminder, index) => {
                                    const dateStatus = getDateStatus(reminder.date);
                                    return (
                                        <TableRow key={reminder.id} className={`group hover:bg-gray-50 ${focusedRowIndex === index ? "bg-blue-50" : ""} ${selectedIds.includes(reminder.id) ? "bg-blue-50/50" : ""}`} onClick={() => setFocusedRowIndex(index)}>
                                            <TableCell className={ROW_DENSITY_STYLES[rowDensity]}>
                                                <Checkbox checked={selectedIds.includes(reminder.id)} onCheckedChange={() => toggleSelect(reminder.id)} onClick={(e) => e.stopPropagation()} />
                                            </TableCell>
                                            {visibleColumns.map(col => (
                                                <TableCell key={col.key} className={ROW_DENSITY_STYLES[rowDensity]}>
                                                    {col.key === "status" && (
                                                        <Badge className={`${getStatusBadge(dateStatus)} font-normal`}>
                                                            <HighlightText text={formatStatus(dateStatus)} search={searchQuery} />
                                                        </Badge>
                                                    )}
                                                    {col.key === "title" && (
                                                        <div className="flex items-center gap-2">
                                                            {reminder.isNotified ? <BellOff className="h-3 w-3 text-gray-400" /> : <Bell className="h-3 w-3 text-blue-500" />}
                                                            <span className="font-medium text-gray-900"><HighlightText text={reminder.title} search={searchQuery} /></span>
                                                        </div>
                                                    )}
                                                    {col.key === "description" && <span className="text-gray-500 truncate max-w-xs block"><HighlightText text={reminder.description || "-"} search={searchQuery} /></span>}
                                            ...
                                            <TableCell className={`${ROW_DENSITY_STYLES[rowDensity]} overflow-visible`}>
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => deleteReminder(reminder.id)}><Trash2 className="h-4 w-4 text-red-500" /></Button></TooltipTrigger><TooltipContent>Delete</TooltipContent></Tooltip>
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    );
