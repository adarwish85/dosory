...
export default function EstimatesPage() {
    ...
    const {
        ...
        estimateStats,
    } = useEstimates({ customerId: customerId || undefined });
    ...
    const processedEstimates = useMemo(() => {
        ...
        if (searchQuery) {
            ...
            result = result.filter(
                (est) => String(est.number).includes(searchQuery) || est.status?.toLowerCase().includes(lowerQuery)
            );
        }
            ...
            result.sort((a, b) => {
                ...
                switch (sortKey) {
                    ...
                    case "date":
                        ...
                        break;
                    case "expiryDate":
                        aVal = a.expiryDate?.toMillis?.() || 0;
                        bVal = b.expiryDate?.toMillis?.() || 0;
                        break;
                    case "status":
                        aVal = a.status || "";
                        bVal = b.status || "";
    ...
    const isAllSelected =
        paginatedEstimates.length > 0 && paginatedEstimates.every((est) => selectedIds.includes(est.id));
    const isSomeSelected = paginatedEstimates.some((est) => selectedIds.includes(est.id)) && !isAllSelected;

    // Export
    const handleExport = () => {
        const dataToExport =
            selectedIds.length > 0 ? estimates.filter((est) => selectedIds.includes(est.id)) : processedEstimates;
        const csv = [
            "Estimate #,Amount,Tax,Date,Expiry,Status",
            ...dataToExport.map(
                (est) =>
                    `"EST-${est.number}","${est.total}","${est.taxTotal || 0}","${formatDate(est.date)}","${formatDate(est.expiryDate)}","${est.status}"`
            ),
        ].join("\n");
    ...
    const handleKeyDown = useCallback(
        (e: KeyboardEvent<HTMLDivElement>) => {
            ...
            switch (e.key) {
                case "ArrowDown":
                    e.preventDefault();
                    setFocusedRowIndex(Math.min(focusedRowIndex + 1, paginatedEstimates.length - 1));
                    break;
                case "ArrowUp":
                    e.preventDefault();
                ...
                case " ":
                    e.preventDefault();
                    toggleSelect(paginatedEstimates[focusedRowIndex].id);
                    break;
            }
        },
                        ...
                        <TableBody>
                            {paginatedEstimates.length === 0 ? (
                                ...
                                paginatedEstimates.map((estimate, index) => (
                                    <TableRow
                                        key={estimate.id}
                                        className={`group hover:bg-gray-50 ${focusedRowIndex === index ? "bg-blue-50" : ""} ${selectedIds.includes(estimate.id) ? "bg-blue-50/50" : ""}`}
                                    ...
                                    >
                                            ...
                                            <TableCell key={col.key} className={ROW_DENSITY_STYLES[rowDensity]}>
                                                {col.key === "number" && (
                                                    <span className="text-blue-600 font-medium">
                                                        <HighlightText
                                                            text={`EST-${String(estimate.number).padStart(6, "0")}`}
                                                            search={searchQuery}
                                                        />
                                                    </span>
                                                )}
                                                ...
                                                {col.key === "tax" && (
                                                    <span className="text-gray-600">
                                                        {formatCurrency(estimate.taxTotal || 0)}
                                                    </span>
                                                )}
                                        ...
                                        <TableCell className={`${ROW_DENSITY_STYLES[rowDensity]} overflow-visible`}>
                                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <Link href={`/dashboard/sales/estimates/${estimate.id}/edit`}>
                                                            <Button variant="ghost" size="icon" className="h-7 w-7">
