...
export function FinanceOverviewWidget({ invoiceStats, estimateStats, proposalStats }: {
    ...
    const invoiceItems = [
        { label: "Draft", count: invoiceStats?.draft || 0, percentage: getPerc(invoiceStats?.draft || 0, invoiceStats?.total || 1), color: "text-gray-500" },
        { label: "Not Sent", count: (invoiceStats?.sent || 0) + (invoiceStats?.viewed || 0), percentage: getPerc((invoiceStats?.sent || 0) + (invoiceStats?.viewed || 0), invoiceStats?.total || 1), color: "text-gray-600" },
        { label: "Unpaid", count: invoiceStats?.partial || 0, percentage: getPerc(invoiceStats?.partial || 0, invoiceStats?.total || 1), color: "text-red-500" },
        { label: "Partially Paid", count: 0, percentage: 0, color: "text-orange-500" }, // Mock if missing
        { label: "Overdue", count: invoiceStats?.overdue || 0, percentage: getPerc(invoiceStats?.overdue || 0, invoiceStats?.total || 1), color: "text-amber-600" },
        { label: "Paid", count: invoiceStats?.paid || 0, percentage: getPerc(invoiceStats?.paid || 0, invoiceStats?.total || 1), color: "text-green-500" }
    ];
    ...
    const estimateItems = [
        ...
        { label: "Not Sent", count: estimateStats?.sent || 0, percentage: getPerc(estimateStats?.sent || 0, estimateStats?.total || 1), color: "text-gray-600" },
        { label: "Sent", count: 0, percentage: 0, color: "text-blue-500" },
    ...
    const proposalItems = [
        ...
        { label: "Open", count: 0, percentage: 0, color: "text-gray-600" },
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="border border-gray-200 rounded-lg p-4">
                            ...
                            <div className="text-lg font-bold text-gray-800">EGP67,208.00</div>
                        </div>
                        <div className="border border-gray-200 rounded-lg p-4">
                            <div className="text-sm font-medium text-gray-600 mb-1">Past Due Invoices</div>
                            <div className="text-lg font-bold text-gray-800">EGP0.00</div>
                        </div>
                        <div className="border border-gray-200 rounded-lg p-4">
                            <div className="text-sm font-medium text-green-600 mb-1">Paid Invoices</div>
                            <div className="text-lg font-bold text-gray-800">EGP530,722.00</div>
                        </div>
                    </div>
                </div>
            </CardContent>
