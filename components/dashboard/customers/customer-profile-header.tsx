...
import { Phone, Mail, Calendar, FileText, CheckSquare, Bell, Receipt } from "lucide-react";
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b bg-white">
            <div>
                <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                    {customer.company}
                    <span className={`text-sm font-normal px-2 py-0.5 rounded-full ${customer.status === 'active' ? 'bg-green-100 text-green-700' :
                ...
                <TooltipProvider delayDuration={0}>
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button variant="outline" size="icon" onClick={() => customer.phone && window.open(`tel:${customer.phone}`)}>
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Button variant="outline" size="icon" onClick={() => window.location.href = `mailto:?subject=Regarding ${customer.company}`}>
