...
async function getStats(orgId: string, period: string) {
...
async function waitForTrigger(ms: number = 2000): Promise<void> {
...
describe("Analytics: Invoice Triggers", () => {
    ...
    it("should increment stats when invoice is created", async () => {
        // Create an invoice
        ...
        expect(stats?.outstandingReceivables).toBe(1000);
    });

    it("should update revenue when payment is applied", async () => {
        // Create invoice
        ...
        expect(stats?.outstandingReceivables).toBe(500);
    });

    it("should not count draft invoices", async () => {
        await db.collection("invoices").add({
            orgId: TEST_ORG_ID,
            status: "draft",
            total: 5000,
        ...
        expect(stats?.invoiceCount || 0).toBe(0);
    });

    it("should decrement stats when invoice is voided", async () => {
        // Create invoice
        const invoiceRef = await db.collection("invoices").add({
            orgId: TEST_ORG_ID,
            status: "sent",
...
describe("Analytics: Customer Triggers", () => {
    const period = "2026-01";
    ...
    it("should increment customer counts on creation", async () => {
    ...
    it("should decrement when customer is archived", async () => {
...
describe("Analytics: Lead Triggers", () => {
    const period = "2026-01";
    ...
    it("should track lead creation and pipeline value", async () => {
        ...
        expect(stats?.leadPipelineValue).toBe(5000);
        expect(stats?.convertedLeads).toBe(0);
    });
    ...
    it("should track lead conversion", async () => {
        ...
        expect(stats?.leadPipelineValue).toBe(0);
    });

    it("should remove pipeline value when lead is lost", async () => {
        const leadRef = await db.collection("leads").add({
            orgId: TEST_ORG_ID,
            status: "qualified",
            name: "Risky Lead",
...
describe("Analytics: Project Triggers", () => {
    const period = "2026-01";
    ...
    it("should track project completion", async () => {
...
describe("Analytics: Multi-tenant Isolation", () => {
    ...
    it("should isolate stats between tenants", async () => {
        ...
        expect(statsA?.totalRevenue).toBe(500);
        expect(statsB?.totalRevenue).toBe(3000);
