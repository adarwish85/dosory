...
function getStatsRef(orgId: string, date: Date = new Date()) {
...
export const onInvoiceWrite = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        const invoiceDate = toDate(after?.date || before?.date || before?.createdAt);
        ...
        const isValidStatus = (status: string) =>
            status && status !== "void" && status !== "cancelled" && status !== "draft";
...
export const onPaymentWrite = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        const paymentDate = toDate(after?.date || before?.date);
...
export const onCustomerWrite = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        const createdAt = toDate(after?.createdAt || before?.createdAt);
        ...
        const isActive = (status: string) => status !== "archived" && status !== "deleted";
        ...
        if (totalDelta === 0 && newDelta === 0) return null;
...
export const onLeadWrite = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        const isConverted = (status: string) => status === "converted" || status === "won";
        const isActive = (status: string) =>
            status && !["converted", "won", "lost", "archived"].includes(status);
...
export const onProjectWrite = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        const getCategory = (status: string): "active" | "completed" | "other" => {
        ...
        if (!before && after) {
            ...
            if (category === "active") activeDelta = 1;
            if (category === "completed") completedDelta = 1;
        }
        // CASE 2: Deletion
        else if (before && !after) {
        ...
        // CASE 3: Update
            ...
            if (beforeCat !== afterCat) {
                ...
                if (beforeCat === "active") activeDelta = -1;
                if (beforeCat === "completed") completedDelta = -1;

                // Increment new category
                if (afterCat === "active") activeDelta += 1;
                if (afterCat === "completed") completedDelta += 1;
            }
        }

        if (activeDelta === 0 && completedDelta === 0) return null;
...
export const dailyAnalyticsSnapshot = functions.pubsub
    .schedule("0 0 * * *")
    .timeZone("UTC")
    .onRun(async (context) => {
...
export const monthlyAnalyticsSummary = functions.pubsub
...
export const recalculateAnalytics = functions.https.onCall(async (data, context) => {
    // Auth check
...
export async function cleanupOldAnalytics(monthsToKeep: number = 24) {
