...
export const onInvoiceWrite = functions.firestore
    ...
    .onWrite(async (change, context) => {
        ...
        if (!orgId) return null; // Orphaned data

        // Determine Month Key (YYYY_MM) to shard aggregates by month
        // Use created date or current date
        const date = after?.date       // Firestore Timestamp
            ? after.date.toDate()
            : (before?.date ? before.date.toDate() : new Date());
...
export const dailyAnalyticsSnapshot = functions.pubsub.schedule("0 0 * * *")
    .timeZone("UTC")
    .onRun(async (context) => {
